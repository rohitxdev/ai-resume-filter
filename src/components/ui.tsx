...
interface SwitchProps
	extends Omit<ComponentProps<"input">, "type" | "onInput"> {}

export const Switch = ({ className, ...rest }: SwitchProps) => {
	return (
		<AriaSwitch
			className={`relative isolate box-content h-5 w-10 rounded-full bg-neutral-600 selected:bg-green-500 p-0.5 duration-100 ${className}`}
		>
			{({ isSelected }) => (
				<div
					className={`absolute top-0.5 left-0.5 z-10 flex size-5 items-center justify-center rounded-full bg-white duration-100 ${isSelected && "translate-x-full"}`}
				>
					{isSelected && (
						<LuCheck className="size-4 stroke-[3] text-green-500" />
					)}
				</div>
			)}
		</AriaSwitch>
...
export const Select = ({
	...
	return (
		<AriaSelect className={`mr-auto ${className}`} {...rest}>
			<Label className="text-black text-xs empty:hidden">{label}</Label>
...
export const AccordionItem = ({ title, description }: AccordionItemProps) => {
	...
	return (
		<div
			className="w-full max-w-[65ch] p-4"
		...
		>
			...
			<div
				className={`grid text-start text-black ${
...
export const InputField = ({
	...
	return (
		<TextField className="grid gap-1" {...rest}>
			<Label className="text-black text-xs empty:hidden">{label}</Label>
			<div
				className={`flex h-9 items-center gap-3 overflow-hidden rounded font-medium outline outline-black/20 duration-100 focus-within:outline-twine-500/80 ${className}`}
			>
				<Input
				...
				{isSensitive && (
					<Button
						className="mr-2 flex items-center justify-center text-black *:size-5"
...
export const MultiButton = ({ options, ...rest }: MultiButtonProps) => {
	return (
		<div className="flex h-9 w-40 items-center divide-x overflow-hidden rounded border bg-twine-500">
			<Button
